...
case class CliffWorld extends POMDP (
  ...
  transitionFunction = (state, actions) => {
  	...
  	State("Q" -> move(qPos, qAction)) + ("S" -> move(sPos, sAction))
  },

  rewardFunction = (state, actions, _) => (agentType) => {
  	val agentPos = state.getAs[(Int, Int)](
      agentType.name
    ) getOrElse (0, 0)
  ...
  observationFunction = (state, actions, statePrime) =>
                        (agentNumber, agentType) =>
    if (agentType == 'Q) state - "S" else state - "Q"
)