...
case class POMDP(
  ...
  agents: List[(AgentType, Int, Int)],

  initialState: State,

  actionsFunction: AgentType => List[Action],

  transitionFunction: (State, JointAction) => State,

  // the Int here is a unique agent number.  Agent numbers are numbered from 1 to n
  rewardFunction: (State, JointAction) => List[(Int, Float)],

  // the Int here is a unique agent number.  Agent numbers are numbered from 1 to n
  observationFunction: (State, JointAction) => List[(Int, State)]
